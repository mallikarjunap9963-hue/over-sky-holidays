  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await enquiriesApi.submitEnquiry({
        ...
        destination: formData.destination || 'General Booking Request',
      ...
      if (res.success || res.status) {
        ...
        if (res.errors) {
          ...
          setError(firstErr || res.message || 'Failed to submit booking inquiry.');
        } else {
          setError(res.message || 'Failed to submit booking inquiry.');
        }
      }
    } catch (err: any) {
      console.error('Booking inquiry error:', err);
      setError('Network error. Unable to send inquiry. Please try again.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="relative z-10">
          ...
          <form onSubmit={handleSubmit} className="mt-5 space-y-2.5">
            ...
            <button
              type="submit"
              disabled={loading}
              className="btn-primary w-full min-h-[42px] rounded-[6px] text-[13.5px] font-bold shadow-[0_12px_24px_rgba(8,83,164,0.18)] font-rubik cursor-pointer gap-2 disabled:opacity-60 flex items-center justify-center"
            >
              {loading ? (
                <>
