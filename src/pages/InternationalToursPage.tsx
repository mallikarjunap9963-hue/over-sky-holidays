...
export function InternationalToursPage() {
  ...
  useEffect(() => {
    ...
    async function loadBanner() {
      try {
        const res = await contentApi.getPageBanner('tours-international');
        if (isMounted && res.banner) {
          setBanner(res.banner);
        }
      } catch (err) {
        console.error("Failed to load international tours banner:", err);
      }
    }
    loadBanner();
    return () => {
        ...
        {loading ? (
          ...
          <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            {tours.map((tour, index) => (
              ...
              >
                <Link
                  to={`/tour/international/${tour.id || tour.slug}`}
