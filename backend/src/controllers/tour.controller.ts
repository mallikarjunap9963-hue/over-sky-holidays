...
export class TourController {
      ...
      if (destination && destination !== 'Select Destination' && Array.isArray(tours)) {
        ...
        tours = tours.filter((t: any) => {
          ...
          const matchLocs = Array.isArray(t.locations)
            ? t.locations.some((l: string) => l.toLowerCase().includes(destLower))
            : false;
  ...
  async searchTours(req: Request, res: Response, next: NextFunction) {
    try {
  ...
  async getTourTypes(req: Request, res: Response, next: NextFunction) {
    try {
  ...
  async getTourFeatures(req: Request, res: Response, next: NextFunction) {
    try {
